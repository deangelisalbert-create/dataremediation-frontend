...
function ReportPanel({ file, onClose }) {
  ...
  const getLink = async (type) => {
    ...
    try {
      ...
      // Ouvrir directement le téléchargement
  ...
  return (
    <div className="fadeUp card" style={{padding:20,position:'sticky',top:80,maxHeight:'calc(100vh - 100px)',overflowY:'auto'}}>
      ...
      <div style={{fontSize:11,color:P.muted,marginBottom:16,padding:'8px 10px',background:P.surface,borderRadius:6,border:`1px solid ${P.border}`}}>
        <div style={{fontWeight:600,color:P.text,marginBottom:2}}>{file.original_name}</div>
        <div>{fmtSize(file.file_size)} · {fmtDate(file.uploaded_at)}</div>
        {file.completed_at && <div style={{color:P.accent}}>✓ Terminé {fmtDate(file.completed_at)}</div>}
      </div>

      {!isDone ? (
        <div style={{textAlign:'center',padding:'32px 0'}}>
          {file.status==='error' ? (
            <><div style={{fontSize:28,marginBottom:8,color:P.danger}}>✗</div><div style={{color:P.danger,fontSize:12}}>{file.error_message||'Analyse échouée'}</div></>
          ) : (
            <><div className="spin" style={{fontSize:28,marginBottom:8,color:P.warn}}>◎</div><div style={{color:P.warn,fontSize:12}}>Analyse en cours…</div><div style={{fontSize:10,color:P.muted,marginTop:4}}>Pseudo · Validation · IA Claude</div></>
          )}
        </div>
      ) : (
        <>
          ...
          {file.row_count > 0 && (
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:16}}>
              {[
                ['Total',    file.row_count,       P.blue],
                ['Taux',     `${file.taux_conformite}%`, P.accent],
                ['Corriger', file.a_corriger,       P.warn],
                ['Bloquants',file.bloquants,        P.danger],
              ].map(([l,v,c],i)=>(
          ...
          <div style={{background:P.surface,border:`1px solid ${P.border}`,borderRadius:8,padding:14}}>
            ...
            <div style={{marginTop:10,fontSize:9,color:P.dim,lineHeight:1.6}}>🔐 Liens signés JWT · {15} min · Via backend sécurisé</div>
          </div>
        </>
      )}
    </div>
